document.addEventListener("DOMContentLoaded", () => {
	...
	/* 动态背景 */
	{
		if (!Joe.IS_MOBILE && Joe.DYNAMIC_BACKGROUND_PC !== "off" && Joe.DYNAMIC_BACKGROUND_PC && !Joe
			...
			$.getScript(window.Joe.THEME_URL + `sources/js/backdrop/${Joe.DYNAMIC_BACKGROUND_PC}`);
		}
		if ((Joe.IS_MOBILE) && (Joe.DYNAMIC_BACKGROUND_WAP !== "off") && (Joe.DYNAMIC_BACKGROUND_WAP) && (!Joe
			.WALLPAPER_BACKGROUND_WAP)) {
			$.getScript(window.Joe.THEME_URL + `sources/js/backdrop/${Joe.DYNAMIC_BACKGROUND_WAP}`);
		}
	}
	...
	/* 全局飘落物 */
	{
		if (Joe.FLOAT_OBJECT != 'off') {
			setTimeout(function () {
				$.getScript(window.Joe.THEME_URL + `sources/js/backdrop/${Joe.FLOAT_OBJECT}`, () => {
	...
	/* 初始化表情功能 */
	{
		if ($(".joe_owo__contain").length && $(".joe_owo__target").length) {
			$.ajax({
				url: window.Joe.THEME_URL + "sources/json/owo.json",
				dataType: "json",
				success(res) {
					let barStr = "";
					let scrollStr = "";
